...
def test_parse_formatted_table(ddl_data):
    clean_data = parse_formatted_table(ddl_data, "activity_table")
    table_columns = clean_data[1]

    assert isinstance(clean_data, list)
    assert clean_data[0] == "activity"
    assert table_columns[0] == ["activity_id", "bifint"]
    assert table_columns[1] == ["code", "varhcar(10)"]
    ...
    assert table_columns[10] == ["move_timeshet", "varchar(1)"]
